...
static int get_keyboard_layout_file(char *layoutFile, DWORD bufferSize) {
    ...
    WCHAR wBuf[KL_NAMELENGTH];
    if (GetKeyboardLayoutName(wBuf)) {
        char kbdName[100];
        size_t outLen;
        wcstombs_s(&outLen, kbdName, sizeof(kbdName), wBuf, _TRUNCATE);
        ...
        if (regPath != NULL) {
            ...
            if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, (LPCSTR) regPath, 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
                const char *regKey = "Layout File";
                if (RegQueryValueExA(hKey, (LPCSTR) regKey, NULL, &varType, (LPBYTE) layoutFile, &bufferSize) == ERROR_SUCCESS) {
                    RegCloseKey(hKey);
            ...
            for (int i = 0; i < new_size; i++) {
                ...
                if (hkl_list[i] != NULL) {
                    ...
                    if (get_keyboard_layout_file(layoutFile, MAX_PATH) == UIOHOOK_SUCCESS) {
                        ...
                        if (GetSystemDirectoryA((LPSTR) systemDirectory, MAX_PATH) != 0) {
                            ...
                            logger(LOG_LEVEL_DEBUG, "%s [%u]: Loading layout for %#p: %s.\n",
                                    __FUNCTION__, __LINE__, hkl_list[i], kbdLayoutFilePath);

                            // Create the new locale item.
                            locale_item = malloc(sizeof(KeyboardLocale));
                            locale_item->id = hkl_list[i];
                            locale_item->library = LoadLibraryA((LPCSTR) kbdLayoutFilePath);
